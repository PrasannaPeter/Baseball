		...
		{   
			...
			$res = PdoLafleur::$monPdo->exec($req);
				...
				{ 
					...
					$req =" select count(id) as nb from administrateur where nom='$login' and mdp='$mdp'";
					$res = PdoLafleur::$monPdo->query($req);
					$laLigne = $res->fetch();
				...
				{ 	
					$req =" select count(*)  from contenir where idProduit='$id'";
					$res = PdoLafleur::$monPdo->query($req);
					$laLigne = $res->fetch();
					...
					if($laLigne[0] > 0)
						...
						else
						{
								...
								$req3 =" delete from produit where id='$id'";
								$res = PdoLafleur::$monPdo->exec($req3);
			...
			//modification
				public function ModifierProduits($id,$Newprix,$Newdescription)
				{ 	
					$req = " UPDATE produit SET description='$Newdescription' ,  prix='$Newprix' WHERE id='$id'";
